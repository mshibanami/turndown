...
describe('TurndownService', () => {
    ...
    it('multiple ps', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe('Lorem\n\nipsum\n\nsit');
    });

    it('em', () => {
        const turndownService = new TurndownService();
        const input = '<em>em element</em>';
        expect.soft(turndownService.turndown(input)).toBe('_em element_');
    });

    it('i', () => {
        const turndownService = new TurndownService();
        const input = '<i>i element</i>';
        expect.soft(turndownService.turndown(input)).toBe('_i element_');
    });

    it('strong', () => {
        const turndownService = new TurndownService();
        const input = '<strong>strong element</strong>';
        expect.soft(turndownService.turndown(input)).toBe('**strong element**');
    });

    it('b', () => {
        const turndownService = new TurndownService();
        const input = '<b>b element</b>';
        expect.soft(turndownService.turndown(input)).toBe('**b element**');
    });

    it('code', () => {
        const turndownService = new TurndownService();
        const input = '<code>code element</code>';
        expect.soft(turndownService.turndown(input)).toBe('`code element`');
    });

    it('code containing a backtick', () => {
        const turndownService = new TurndownService();
        const input = '<code>There is a literal backtick (`) here</code>';
        expect.soft(turndownService.turndown(input)).toBe('``There is a literal backtick (`) here``');
    });

    it('code containing three or more backticks', () => {
        const turndownService = new TurndownService();
        const input = '<code>here are three ``` here are four ```` that\'s it</code>';
        expect.soft(turndownService.turndown(input)).toBe("`here are three ``` here are four ```` that's it`");
    });

    it('code containing one or more backticks', () => {
        const turndownService = new TurndownService();
        const input = '<code>here are three ``` here are four ```` here is one ` that\'s it</code>';
        expect.soft(turndownService.turndown(input)).toBe("``here are three ``` here are four ```` here is one ` that's it``");
    });

    it('code starting with a backtick', () => {
        const turndownService = new TurndownService();
        const input = '<code>`starting with a backtick</code>';
        expect.soft(turndownService.turndown(input)).toBe('`` `starting with a backtick ``');
    });
    ...
    it('code containing markdown syntax', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("`_emphasis_`");
    });
    ...
    it('h1', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Level One Heading\n=================");
    });

    it('escape = when used as heading', () => {
        const turndownService = new TurndownService();
        const input = "===";
        expect.soft(turndownService.turndown(input)).toBe("\\===");
    });

    it('not escaping = outside of a heading', () => {
        const turndownService = new TurndownService();
        const input = "A sentence containing =";
        expect.soft(turndownService.turndown(input)).toBe("A sentence containing =");
    });

    it('h1 as atx', () => {
        const turndownService = new TurndownService({ "headingStyle": "atx" });
        const input = "<h1>Level One Heading with ATX</h1>";
        expect.soft(turndownService.turndown(input)).toBe("# Level One Heading with ATX");
    });

    it('h2', () => {
        const turndownService = new TurndownService();
        const input = "<h2>Level Two Heading</h2>";
        expect.soft(turndownService.turndown(input)).toBe("Level Two Heading\n-----------------");
    });

    it('h2 as atx', () => {
        const turndownService = new TurndownService({ "headingStyle": "atx" });
        const input = "<h2>Level Two Heading with ATX</h2>";
        expect.soft(turndownService.turndown(input)).toBe("## Level Two Heading with ATX");
    });

    it('h3', () => {
        const turndownService = new TurndownService();
        const input = "<h3>Level Three Heading</h3>";
        expect.soft(turndownService.turndown(input)).toBe("### Level Three Heading");
    });

    it('heading with child', () => {
        const turndownService = new TurndownService();
        const input = "<h4>Level Four Heading with <code>child</code></h4>";
        expect.soft(turndownService.turndown(input)).toBe("#### Level Four Heading with `child`");
    });

    it('invalid heading', () => {
        const turndownService = new TurndownService();
        const input = "<h7>Level Seven Heading?</h7>";
        expect.soft(turndownService.turndown(input)).toBe("Level Seven Heading?");
    });
    ...
    it('br', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("More  \nafter the break");
    });
    ...
    it('img with no alt', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("![](http://example.com/logo.png)");
    });

    it('img with relative src', () => {
        const turndownService = new TurndownService();
        const input = "<img src=\"logo.png\">";
        expect.soft(turndownService.turndown(input)).toBe("![](logo.png)");
    });

    it('img with alt', () => {
        const turndownService = new TurndownService();
        const input = "<img src=\"logo.png\" alt=\"img with alt\">";
        expect.soft(turndownService.turndown(input)).toBe("![img with alt](logo.png)");
    });
    ...
    it('img with a new line in alt', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("![img with\nalt](logo.png)");
    });
    ...
    it('img with new lines in title', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("![](logo.png \"the\ntitle\")");
    });

    it('a', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://example.com\">An anchor</a>";
        expect.soft(turndownService.turndown(input)).toBe("[An anchor](http://example.com)");
    });

    it('a with title', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://example.com\" title=\"Title for link\">An anchor</a>";
        expect.soft(turndownService.turndown(input)).toBe("[An anchor](http://example.com \"Title for link\")");
    });

    it('a with multiline title', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://example.com\" title=\"Title for\n    \n    link\">An anchor</a>";
        expect.soft(turndownService.turndown(input)).toBe("[An anchor](http://example.com \"Title for\nlink\")");
    });

    it('a with quotes in title', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://example.com\" title=\"&quot;hello&quot;\">An anchor</a>";
        expect.soft(turndownService.turndown(input)).toBe("[An anchor](http://example.com \"\\\"hello\\\"\")");
    });

    it('a with parenthesis in query', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://example.com?(query)\">An anchor</a>";
        expect.soft(turndownService.turndown(input)).toBe("[An anchor](http://example.com?\\(query\\))");
    });

    it('a without a src', () => {
        const turndownService = new TurndownService();
        const input = "<a id=\"about-anchor\">Anchor without a title</a>";
        expect.soft(turndownService.turndown(input)).toBe("Anchor without a title");
    });

    it('a with a child', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://example.com/code\">Some <code>code</code></a>";
        expect.soft(turndownService.turndown(input)).toBe("[Some `code`](http://example.com/code)");
    });

    it('a reference', () => {
        const turndownService = new TurndownService({ "linkStyle": "referenced" });
        const input = "<a href=\"http://example.com\">Reference link</a>";
        expect.soft(turndownService.turndown(input)).toBe("[Reference link][1]\n\n[1]: http://example.com");
    });
    ...
    it('a reference with collapsed style', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("[Reference link with collapsed style][]\n\n[Reference link with collapsed style]: http://example.com");
    });
    ...
    it('a reference with shortcut style', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("[Reference link with shortcut style]\n\n[Reference link with shortcut style]: http://example.com");
    });

    it('pre/code block', () => {
        const turndownService = new TurndownService();
        const input = read('pre-code-block.html');
        expect.soft(turndownService.turndown(input)).toBe(read('pre-code-block.md'));
    });

    it('multiple pre/code blocks', () => {
        const turndownService = new TurndownService();
        const input = read('multiple-pre-code-blocks.html');
        expect.soft(turndownService.turndown(input)).toBe(read('multiple-pre-code-blocks.md'));
    });

    it('pre/code block with multiple new lines', () => {
        const turndownService = new TurndownService();
        const input = read('pre-code-block-with-multiple-new-lines.html');
        expect.soft(turndownService.turndown(input)).toBe(read('pre-code-block-with-multiple-new-lines.md'));
    });
    ...
    it('escaping ~~~', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("\\~~~ foo");
    });
    ...
    it('fenced pre/code block with language', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("```ruby\ndef a_fenced_code block; end\n```");
    });
    ...
    it('list spacing', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("A paragraph.\n\n1.  Ordered list item 1\n2.  Ordered list item 2\n3.  Ordered list item 3\n\nAnother paragraph.\n\n*   Unordered list item 1\n*   Unordered list item 2\n*   Unordered list item 3");
    });

    it('ul', () => {
        const turndownService = new TurndownService();
        const input = "<ul>\n      <li>Unordered list item 1</li>\n      <li>Unordered list item 2</li>\n      <li>Unordered list item 3</li>\n    </ul>";
        expect.soft(turndownService.turndown(input)).toBe("*   Unordered list item 1\n*   Unordered list item 2\n*   Unordered list item 3");
    });
    ...
    it('ul with paragraph', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("*   List item with paragraph\n    \n*   List item without paragraph");
    });
    ...
    it('leading whitespace in heading', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("### h3 with leading whitespace");
    });
    ...
    it('whitespace between inline elements', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("I [need](http://example.com/need) [more](http://www.example.com/more) spaces!");
    });
    ...
    it('whitespace in inline elements', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Text with no space after the period. _Text in em with leading/trailing spaces_ **text in strong with trailing space**");
    });
    ...
    it('whitespace in nested inline elements', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Text at root **[link text with trailing space in strong](http://www.example.com)** more text at root");
    });

    it('blank inline elements', () => {
        const turndownService = new TurndownService();
        const input = "Text before blank em … <em></em> text after blank em";
        expect.soft(turndownService.turndown(input)).toBe("Text before blank em … text after blank em");
    });

    it('blank block elements', () => {
        const turndownService = new TurndownService();
        const input = "Text before blank div … <div></div> text after blank div";
        expect.soft(turndownService.turndown(input)).toBe("Text before blank div …\n\ntext after blank div");
    });
    ...
    it('whitespace between blocks', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Content in a nested div\n\nContent in another div");
    });

    it('escaping backslashes', () => {
        const turndownService = new TurndownService();
        const input = "backslash \\";
        expect.soft(turndownService.turndown(input)).toBe("backslash \\\\");
    });

    it('escaping headings with #', () => {
        const turndownService = new TurndownService();
        const input = "### This is not a heading";
        expect.soft(turndownService.turndown(input)).toBe("\\### This is not a heading");
    });
    ...
    it('escaping em markdown with *', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("To add emphasis, surround text with \\*. For example: \\*this is emphasis\\*");
    });

    it('escaping em markdown with _', () => {
        const turndownService = new TurndownService();
        const input = "To add emphasis, surround text with _. For example: _this is emphasis_";
        expect.soft(turndownService.turndown(input)).toBe("To add emphasis, surround text with \\_. For example: \\_this is emphasis\\_");
    });

    it('not escaping within code', () => {
        const turndownService = new TurndownService();
        const input = "<pre><code>def this_is_a_method; end;</code></pre>";
        expect.soft(turndownService.turndown(input)).toBe("def this_is_a_method; end;");
    });

    it('escaping strong markdown with *', () => {
        const turndownService = new TurndownService();
        const input = "To add strong emphasis, surround text with **. For example: **this is strong**";
        expect.soft(turndownService.turndown(input)).toBe("To add strong emphasis, surround text with \\*\\*. For example: \\*\\*this is strong\\*\\*");
    });
    ...
    it('escaping strong markdown with _', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("To add strong emphasis, surround text with \\_\\_. For example: \\_\\_this is strong\\_\\_");
    });
    ...
    it('escaping hr markdown with _', () => {
        const turndownService = new TurndownService();
        const input = "_ _ _";
        expect.soft(turndownService.turndown(input)).toBe("\\_ \\_ \\_");
    });
    ...
    it('escaping ol markdown', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("1984\\. by George Orwell");
    });

    it('not escaping . outside of an ol', () => {
        const turndownService = new TurndownService();
        const input = "1984.George Orwell wrote 1984.";
        expect.soft(turndownService.turndown(input)).toBe("1984.George Orwell wrote 1984.");
    });

    it('escaping ul markdown *', () => {
        const turndownService = new TurndownService();
        const input = "* An unordered list item";
        expect.soft(turndownService.turndown(input)).toBe("\\* An unordered list item");
    });
    ...
    it('not escaping - outside of a ul', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Hello-world, 45 - 3 is 42");
    });

    it('not escaping + outside of a ul', () => {
        const turndownService = new TurndownService();
        const input = "+1 and another +";
        expect.soft(turndownService.turndown(input)).toBe("+1 and another +");
    });

    it('escaping *', () => {
        const turndownService = new TurndownService();
        const input = "You can use * for multiplication";
        expect.soft(turndownService.turndown(input)).toBe("You can use \\* for multiplication");
    });

    it('escaping ** inside strong tags', () => {
        const turndownService = new TurndownService();
        const input = "<strong>**test</strong>";
        expect.soft(turndownService.turndown(input)).toBe("**\\*\\*test**");
    });

    it('escaping _ inside em tags', () => {
        const turndownService = new TurndownService();
        const input = "<em>test_italics</em>";
        expect.soft(turndownService.turndown(input)).toBe("_test\\_italics_");
    });

    it('unnamed case', () => {
        const turndownService = new TurndownService();
        const input = "> Blockquote in markdown";
        expect.soft(turndownService.turndown(input)).toBe("\\> Blockquote in markdown");
    });
    ...
    it('unnamed case', () => {
        const turndownService = new TurndownService();
        const input = "42 > 1";
        expect.soft(turndownService.turndown(input)).toBe("42 > 1");
    });

    it('escaping code', () => {
        const turndownService = new TurndownService();
        const input = "`not code`";
        expect.soft(turndownService.turndown(input)).toBe("\\`not code\\`");
    });

    it('escaping []', () => {
        const turndownService = new TurndownService();
        const input = "[This] is a sentence with brackets";
        expect.soft(turndownService.turndown(input)).toBe("\\[This\\] is a sentence with brackets");
    });

    it('escaping [', () => {
        const turndownService = new TurndownService();
        const input = "<a href=\"http://www.example.com\">c[iao</a>";
        expect.soft(turndownService.turndown(input)).toBe("[c\\[iao](http://www.example.com)");
    });

    it('escaping * performance', () => {
        const turndownService = new TurndownService();
        const input = "fasdf *883 asdf wer qweasd fsd asdf asdfaqwe rqwefrsdf";
        expect.soft(turndownService.turndown(input)).toBe("fasdf \\*883 asdf wer qweasd fsd asdf asdfaqwe rqwefrsdf");
    });

    it('escaping multiple asterisks', () => {
        const turndownService = new TurndownService();
        const input = "<p>* * ** It aims to be*</p>";
        expect.soft(turndownService.turndown(input)).toBe("\\* \\* \\*\\* It aims to be\\*");
    });
    ...
    it('escaping delimiters around short words and numbers', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("\\_Really\\_? Is that what it \\_is\\_? A \\*\\*2000\\*\\* year-old computer?");
    });
    ...
    it('blank inline elements', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Hello world");
    });
    ...
    it('preformatted code with leading whitespace', () => {
        ...
        expect.soft(turndownService.turndown(input)).toBe("Four spaces `    make an indented code block in Markdown`");
    });

    it('preformatted code with trailing whitespace', () => {
        const turndownService = new TurndownService({ "preformattedCode": true });
        const input = "<code>A line break  </code> <b> note the spaces</b>";
        expect.soft(turndownService.turndown(input)).toBe("`A line break  ` **note the spaces**");
    });

    it('preformatted code tightly surrounded', () => {
        const turndownService = new TurndownService({ "preformattedCode": true });
        const input = "<b>tight</b><code>code</code><b>wrap</b>";
        expect.soft(turndownService.turndown(input)).toBe("**tight**`code`**wrap**");
    });

    it('preformatted code loosely surrounded', () => {
        const turndownService = new TurndownService({ "preformattedCode": true });
        const input = "<b>not so tight </b><code>code</code><b> wrap</b>";
        expect.soft(turndownService.turndown(input)).toBe("**not so tight** `code` **wrap**");
    });

    it('preformatted code with newlines', () => {
        const turndownService = new TurndownService({ "preformattedCode": true });
        const input = "<code>\n\n nasty\ncode\n\n</code>";
        expect.soft(turndownService.turndown(input)).toBe("`    nasty code   `");
    });
